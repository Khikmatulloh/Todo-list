...
$taskObj  = new Task(); // Variable name collision fixed
$todoList = $taskObj->getAll();
?>
...
<div class="list-group list-group-flush">
    ...
    foreach ($todoList as $task) {
        ...
        echo "        <label class='form-check-label $strike' for='task-{$task['id']}'>{$task['task']}</label>";
        echo "    </a>";
        echo "    <a href='?delete={$task['id']}' type='button' class='p-2'><i class='fa-solid fa-trash text-danger'></i></a>";
        echo "    <a href='?update={$task['id']}' type='button' class='p-2'><i class='fa-solid fa-pen text-primary'></i></a>"; // Add update button
        echo "</div>";
    } ?>
</div>
